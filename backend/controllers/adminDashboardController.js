...
console.log('stats', stats)
            if (!stats) {
                return res.status(404).json({
                    success: false,
                    message: 'No dashboard statistics found'
                });
            }
...
async function calculateFinancialStats() {
    try {
        ...
        const feeCollectionRate = totalFees.length > 0
...
async function calculateCommunicationStats() {
    try {
        ...
        const engagementRate = totalParents > 0
            ? (activeParents / totalParents) * 100
            : 0;
...
async function calculateAttendanceStats() {
    try {
        ...
        return {
            students: {
                today: {
                    present: studentAttendanceToday.find(s => s._id === 'present')?.count || 0,
                    absent: studentAttendanceToday.find(s => s._id === 'absent')?.count || 0,
                    pending: studentAttendanceToday.find(s => s._id === 'pending')?.count || 0
                },
                monthly: {
                    present: studentAttendanceMonthly.find(s => s._id === 'present')?.count || 0,
                    absent: studentAttendanceMonthly.find(s => s._id === 'absent')?.count || 0,
                    pending: studentAttendanceMonthly.find(s => s._id === 'pending')?.count || 0
                },
                trend: attendanceTrend
            },
            teachers: {
                today: {
                    present: teacherAttendanceToday.find(t => t._id === 'present')?.count || 0,
                    absent: teacherAttendanceToday.find(t => t._id === 'absent')?.count || 0,
                    pending: teacherAttendanceToday.find(t => t._id === 'pending')?.count || 0
                },
                monthly: {
                    present: teacherAttendanceMonthly.find(t => t._id === 'present')?.count || 0,
                    absent: teacherAttendanceMonthly.find(t => t._id === 'absent')?.count || 0,
                    pending: teacherAttendanceMonthly.find(t => t._id === 'pending')?.count || 0
                }
            }
        };
    } catch (error) {
